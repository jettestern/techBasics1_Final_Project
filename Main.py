...
# ---------- Pygame Receipt Function ----------

def draw_receipt(screen, font, bowls, customer_name):
    screen.fill((255, 255, 255))
    y = 20

    title = font.render("RECEIPT - Green Bowls Restaurant", True, (0, 128, 0))
    screen.blit(title, (20, y))
    y += 40

    # Show customer name and time
    time_text = datetime.utcnow().strftime("%d/%m/%Y %H:%M:%S UTC")
    name_text = font.render(f"Customer: {customer_name}", True, (0, 0, 0))
    time_text_surface = font.render(f"Time: {time_text}", True, (0, 0, 0))
    screen.blit(name_text, (20, y))
    y += 30
    screen.blit(time_text_surface, (20, y))
    y += 40

    total_amount = 0
    for i, bowl in enumerate(bowls, start=1):
        header = font.render(f"Bowl #{i}:", True, (0, 0, 0))
    ...
    total_text = font.render(f"Total: €{total_amount:.2f}", True, (0, 128, 0))
    screen.blit(total_text, (20, y))

    y += 50
    info_text = font.render("Close window to exit.", True, (100, 100, 100))
